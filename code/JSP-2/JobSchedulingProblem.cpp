...
JobSchedulingProblem::JobSchedulingProblem(const Instance &t_instance, double t_Gamma)
    : m_instance(t_instance),
      ...
      m_x(idol::Var::make_vector<1>(m_env, Dim<1>(m_job_occurrences.size()), 0, 1, Binary, "x")),
...
Model JobSchedulingProblem::create_master_problem() {
    ...
    const unsigned int n_jobs = m_instance.n_jobs();
    const unsigned int n_job_occurrences = m_job_occurrences.size();
    ...
    std::vector<Expr<Var, Var>> sum_x_k(n_jobs);
    for (unsigned int k = 0 ; k < n_job_occurrences ; ++k) {
        sum_x_k[m_job_occurrences[k].parent->index] += m_x[k];
    }

    for (unsigned int j = 0 ; j < n_jobs ; ++j) {
        result.add_ctr(sum_x_k[j] <= 1);
    }

    result.set_obj_expr(m_theta + idol_Sum(j, Range(m_instance.n_jobs()), m_instance.job(j).weight * sum_x_k[j]));
                            ...
                            DantzigWolfeDecomposition(m_annotation)
                                    ...
                                    .with_default_sub_problem_spec(
                                            DantzigWolfe::SubProblem()
                                                    .add_optimizer(create_gurobi().with_time_limit(1))
                                                    .add_optimizer(create_gurobi().with_time_limit(5))
...
JobSchedulingProblem::add_scenario_to_master_problem(Model &t_master,
    ...
    for (unsigned int k = 1 ; k < n_job_occurrences ; ++k) {
        ...
        const double tau_k = is_attacked(job_occurrence.parent->index) * m_percentage_increase * p_k;

        Ctr c(m_env, t[k] - t[k-1] - p_k * y[k] - tau_k * y[k] >= 0);
        c.set(m_annotation, t_iteration);
        t_master.add(c);
    }
    ...
    for (unsigned int k = 0 ; k < n_job_occurrences ; ++k) {
        ...
        Ctr c(m_env, t[k] - p_k * y[k] - tau_k * y[k] - m_big_M[k] * y[k] >= job_occurrence.release_date - m_big_M[k]);
        c.set(m_annotation, t_iteration);
        t_master.add(c);
    }
...
Solution::Primal JobSchedulingProblem::compute_worst_case_scenario(const Model &t_master,
    ...
    // Linking constraints
    for (unsigned int k = 0 ; k < n_job_occurrences ; ++k) {
        const double x_val = std::round(t_first_stage_solution.get(m_x[k]));
        separation.add_ctr(y[k] <= x_val);
    }
