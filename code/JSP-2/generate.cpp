...
#include <iostream>
#include <fstream>
#include "../JSP/Instance.h"

int main(int t_argc, const char** t_argv) {

    const std::string destination_folder = "/home/henri/Research/AE-using-column-generation-in-column-and-constraint-generation/code/JSP2/data";
    const auto n_instances_per_config = 5;
    const auto n_values = { 30, 40 };
        ...
        for (auto R : R_values) {
            for (auto D : D_values) {
                ...
                for (unsigned int i = 0 ; i < n_instances_per_config ; ++i) {
                    ...
                    std::ofstream file(std::string(destination_folder).append("/").append(filename));
